    ...
    function normalize(items) {
        return items.map(item => {
            return {
                ...
                thumbnails: {
                    small: item.thumbnails.default.url,
                    medium: item.thumbnails.high.url,
                    large: item.thumbnails.maxres.url,
                },
...
async function fetchVideoDetails(session, videoId) {
    ...
    const res = await fetchAuth(videoDetailsUrl, session.accessToken);
    const data = await res.json();
